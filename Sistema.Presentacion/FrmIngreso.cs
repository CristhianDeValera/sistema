...
namespace Sistema.Presentacion
{
    ...
    public partial class FrmIngreso : MetroFramework.Forms.MetroForm
    {
        ...
        {
            //Este metodo ADD recibe 2 parametros el nombre de la columna y el tipo de dato de la columna
        ...
        {
            //para evitar que vuelva a insertar el mismo registro declaramos un  bool y un foreach
            bool Agregar = true;
        ...
        private void BtnInsertar_Click(object sender, EventArgs e)
        {
            ...
            {
                string Rpta = "";
                if (TxtIdProveedor.Text.Trim() == string.Empty || TxtImpuesto.Text == string.Empty || TxtNumComprobante.Text == string.Empty || DgvDetalle.Rows.Count == 0)
                {
                    this.MensajeError("Falta ingresar algunos datos, serán remarcados.");
                    ErrorIcono.SetError(TxtIdProveedor, "Ingrese un proveedor");
                    ErrorIcono.SetError(TxtImpuesto, "Ingrese un impuesto");
                    ErrorIcono.SetError(TxtNumComprobante, "Ingrese un comprobante");
                    ErrorIcono.SetError(DgvDetalle, "Ingrese al menos un detalle");
                }
                else
                {
                    Rpta = NIngreso.Insertar(Convert.ToInt32(TxtIdProveedor.Text), Variables.IdUsuario, lblcomprobante.Text, TxtSerieComprobante.Text.Trim(), TxtNumComprobante.Text.Trim(), Convert.ToDecimal(TxtImpuesto.Text), Convert.ToDecimal(TxtTotal.Text), DtDetalle);
                    if (Rpta.Equals("OK"))
                    {
